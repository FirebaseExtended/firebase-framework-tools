...
if (packageFromRef && !lernaList.find(it => basename(it.location) === packageFromRef)) {
    throw `Lerna didn't find ${packageFromRef} in this workspace`;
}
...
for (const lerna of lernaList) {
    ...
    if (packageFromRef && packageFromRef !== basename(lerna.location)) continue;
    if (versionFromRef && versionFromRef.split('-')[0] !== lerna.version) {
        throw `Cowardly refusing to publish ${lerna.name}@${versionFromRef} from ${lerna.version}, version needs to be bumped in source.`;
    }
