...
const { handle: frameworkHandle } = dirname ? (await import(`./${dirname}/${basename}.js`)) as Server : { handle: undefined };

// TODO move to middleware
const firebaseAware = usingFirebaseJsSdk ? await import('./firebase-aware.js') : undefined;

export const handle = frameworkHandle ? firebaseAware?.handleFactory(frameworkHandle) || frameworkHandle : (req: Request, res: Response) => {
