...
program
    ...
    const versionStackRank = [
        ...
        `^${version.major}.${version.minor}.0 <= ${version.major}.${version.minor + 1}.0`,
    ...
    if (npmInfo.error)
        npmInfo = JSON.parse(spawnSync("npm", ["info", adapterName, "--json"]).stdout.toString());
    if (npmInfo.error) {
        throw new Error(npmInfo.error.detail);
    }
    const adapterVersion = semverParse(npmInfo.version);
    if (!adapterVersion)
        throw new Error(`Unable to parse ${adapterVersion}`);
    // TODO colors
