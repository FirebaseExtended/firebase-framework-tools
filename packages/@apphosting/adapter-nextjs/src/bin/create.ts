...
export async function create(projectDirectory=process.argv[2], cwd=process.cwd()) {
  return await new Promise<void>((resolve, reject) => {
    ...
    const args = ['-y', '-p', `create-next-app@${CREATE_NEXT_APP_VERSION}`, 'create-next-app', projectDirectory, `--use-${packageManager}`, typescript ? Options.typescript : Options.javascript];
