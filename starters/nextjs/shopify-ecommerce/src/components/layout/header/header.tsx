...
import { handleCustomerLogout } from '@/lib/actions/auth'
...
export default function Header({ navigation, cart, customerAccessToken }: Props) {
        ...
        <div className="max-w-screen-2xl mx-auto flex gap-10 items-center h-[60px]">
          ...
          >
            <div className="max-lg:mb-5 w-full lg:max-w-96 lg:ml-auto">
              <Combobox
                onChange={(value: { productSlug?: string }) => {
                  if (value?.productSlug) {
                    push(`/product/${value.productSlug}`)
                    setQuery('')
                    setSearchResults([])
                    setMobileNavOpen(false)
                  ...
                  >
                    {loading ? (
                      ...
                      searchResults.map((result) => (
                        <ComboboxOption key={result.productID} value={result}>
